...
In this hands-on exercise, I will use R to plot static and interactive heatmap for visualising and analysing multivariate data.
...
### 5.3.3 Transforming the dataframe into a matrix
...
In this section, I will plot static heatmaps by using **heatmap()** of *R Stats* package.
...
-   The order of both rows and columns are different compared to the native wh_matrix. This is because heatmap does a reordering using clusterisation: it calculates the distance between each pair of rows and columns and tries to order them by similarity. The corresponding dendrogram are provided beside the heatmap.

Here, red cells denotes small values. However, the Happiness Score variable has relatively higher values, which makes the other variables with small values all look the same. As such, the heatmap is not really informative.

Thus, we need to normalize this matrix. This is done using the *scale* argument. It can be applied to rows or to columns depending on your needs.
...
[Introduction to Heatmaply](https://cran.r-project.org/web/packages/heatmaply/vignettes/heatmaply.html) is useful in providing an overall understanding of the features and functions of Heatmaply package. The [user manual](https://cran.r-project.org/web/packages/heatmaply/heatmaply.pdf)of the package will also come in handy for reference purposes.

In this section, I will use **heatmaply** to design an interactive cluster heatmap, with wh_matrix as the input data.
