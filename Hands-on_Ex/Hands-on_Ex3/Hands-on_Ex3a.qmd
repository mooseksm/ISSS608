...
::: callout-note
## **Updates - notes from Prof Kam**
For the first segment of the code above, note to always indicate the dataset used before pasting the customised labels i.e. exam_data$ID, "\n" is to have the "Class" label in a separate line. Note the style of coding, cleaner to have "tooltip=exam_data$tooltip" instead of having the detailed code chunk (first segment of code above) within.
:::
...
::: callout-note
## **Updates - notes from Prof Kam** 
Advise to hover over with tooltips data to indicate what the highlighted dots are when hovered. In this case, this is adding tooltips. This is carried out by adding the first segment in the code above i.e. exam_data$tooltip <- c(paste0("Class = ",exam_data$CLASS)) and indicating "tooltip=exam_data\$tooltip" within aes in the p object, or more simply add "tooltip=ID".
:::
