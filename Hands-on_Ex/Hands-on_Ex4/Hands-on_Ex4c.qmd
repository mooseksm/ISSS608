...
Visualising uncertainty is relatively new in statistical graphics. In this chapter, we will create statistical graphics for visualising uncertainty. By the end of this chapter, we will:
...
### 4.3.3 Visualising the uncertainty of point estimates with interactive error bars
...
### 4.4.1 Visualising the uncertainty of point estimates: ggdist methods
...
::: callout-note
This function comes with many arguments, read the syntax reference for more details.
:::
...
### 4.4.2 Visualising the uncertainty of point estimates: ggdist methods

Making over the plot above by showing 95% and 99% confidence intervals respectively:
...
exam %>%
  ggplot(aes(x = RACE, 
             y = MATHS)) +
  stat_pointinterval(.width=c(0.95,0.99), show.legend = FALSE, position="dodgejust") + 
  labs(
    title = "Visualising confidence intervals of mean math score",
    subtitle = "Mean Point + Multiple-interval plot")
```
...
### 4.4.3 Visualising the uncertainty of point estimates: ggdist methods
...
As the step had been performed in an earlier step, we will not repeat running this code (indicated below for reference):
...
## 4.6 Visualising Uncertainty with Hypothetical Outcome Plots (HOPs)
